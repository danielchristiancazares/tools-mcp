...
- Code structure is clean and modular: transport (MCP) logic resides in `src/main.rs`, OpenAI/vector store orchestration in `src/lib.rs`, CodeQuery logic in `src/codequery/*`, and WebFetch functionality in `src/webfetch/*`.
- Error handling uses `anyhow::Result` with contextual messages; retries and backoff are in place for vector store reindexing (`src/lib.rs:1029`).
...
- **SSRF bypass via hostname resolution** (`src/webfetch/http.rs:62`)
...
- **Vector store wait scope** (`src/lib.rs:646`)
...
- **Robots cache ergonomics** (`src/webfetch/http.rs:132`)
...
| File | Summary |
| --- | --- |
| `src/webfetch/http.rs` | Made `validate_url_ssrf` async, added DNS resolution + private IP rejection, documented rationale, and awaited validation in `fetch_document`. |
| `src/lib.rs` | Replaced `split('/').last()` with `rsplit('/').next()` to avoid full iterator traversal. Added comment explaining choice. |
| `tests/integration_test.rs` | Adjusted `.args` calls, converted alias list to array, replaced `expect(format!(...))` with `unwrap_or_else(|| panic!(...))` for clippy compliance. |
