...
- In scope:
  - Runtime MCP server and tool routing in `apps/tools-mcp-server/src/main.rs`, `apps/tools-mcp-server/src/mcp_server.rs`, and `apps/tools-mcp-server/src/composition.rs`.
...
## System model
### Primary components
- `MCP transport/router`: Reads JSON-RPC from stdin/stdout and dispatches `mcp/tools/call` requests without an internal authn/authz layer. Evidence anchors: `apps/tools-mcp-server/src/main.rs::main`, `apps/tools-mcp-server/src/mcp_server.rs::dispatch_jsonrpc_request`.
- `Tool registry/control plane`: Registers all callable tools, including mutating git, file, network, and process-execution tools. Evidence anchors: `apps/tools-mcp-server/src/composition.rs::build_tool_registry`, `crates/tools-mcp-core/src/tool_registry.rs::ToolRegistry::call`.
- `Local filesystem and git layer`: Provides `Read`, `Edit`, `Write`, `Delete`, `Move`, `Copy`, `ListDir`, `Glob`, and multiple git helpers over caller-supplied paths and working directories. Evidence anchors: `crates/tools-mcp-local/src/tools/write.rs::handle_write`, `crates/tools-mcp-local/src/tools/delete.rs::handle_delete`, `crates/tools-mcp-local/src/tools/fileops.rs::handle_move`, `crates/tools-mcp-local/src/smart_file_edit/mod.rs::handle_edit`, `crates/tools-mcp-git/src/git/handlers/mutating.rs`.
- `Process execution layer`: Executes arbitrary PowerShell commands and git subprocesses with timeouts and output caps. Evidence anchors: `crates/tools-mcp-local/src/tools/pwsh.rs::execute_pwsh`, `crates/tools-mcp-core/src/process_utils.rs::run_pwsh_command`, `crates/tools-mcp-git/src/git/mod.rs::run_git`.
- `WebFetch pipeline`: Fetches remote pages over HTTP or headless Chrome with SSRF validation, robots checks, extraction, chunking, and disk caching. Evidence anchors: `crates/tools-mcp-webfetch/src/webfetch_tool.rs::handle_webfetch`, `crates/tools-mcp-webfetch/src/webfetch/mod.rs::run_fetch`, `crates/tools-mcp-webfetch/src/webfetch/http.rs::fetch_document`, `crates/tools-mcp-webfetch/src/webfetch/browser.rs::spawn_browser`.
- `CodeQuery/OpenAI integration`: Auto-discovers local files, resolves or creates vector stores, uploads indexable files to OpenAI, and queries them. Evidence anchors: `crates/tools-mcp-codequery/src/tool_handler.rs::handle_code_query`, `crates/tools-mcp-codequery/src/tool_handler.rs::discover_default_file_paths`, `crates/openai-file-search-core/src/files.rs::upload_file`, `crates/openai-file-search-core/src/reindex.rs::code_query`.
...
- Trusted local MCP client -> MCP server router
  ...
  - Validation and enforcement: request parsing and tool schema validation only; no in-repo caller authentication or authorization. Evidence anchors: `apps/tools-mcp-server/src/main.rs::main`, `apps/tools-mcp-server/src/mcp_server.rs::dispatch_jsonrpc_request`.
- MCP server -> local filesystem and git worktree
  ...
  - Validation and enforcement: no central confinement to current working directory/subdirectories for mutation paths. Evidence anchors: `crates/tools-mcp-local/src/tools/write.rs::handle_write`, `crates/tools-mcp-local/src/tools/delete.rs::handle_delete`, `crates/tools-mcp-local/src/tools/fileops.rs::handle_move`, `crates/tools-mcp-local/src/smart_file_edit/mod.rs::apply_snippet_edit_impl`, `crates/tools-mcp-git/src/git/mod.rs::run_git`.
- MCP server -> PowerShell process
  ...
  - Validation and enforcement: no in-repo command allowlist or env-gate check found; deployment gate is external. Evidence anchors: `crates/tools-mcp-local/src/tools/pwsh.rs::execute_pwsh`, `crates/tools-mcp-core/src/process_utils.rs::run_pwsh_command`.
...
- MCP server -> OpenAI API via CodeQuery
  ...
  - Validation and enforcement: no secret-aware upload policy or approval step before sending repo contents off-host. Evidence anchors: `crates/tools-mcp-codequery/src/tool_handler.rs::handle_code_query`, `crates/tools-mcp-codequery/src/tool_handler.rs::discover_default_file_paths`, `crates/openai-file-search-core/src/files.rs::upload_file`, `crates/openai-file-search-core/src/reindex.rs::code_query`.
- MCP server -> local cache and browser state
  ...
  - Validation and enforcement: web cache has no expiry or size cap; browser runs with `--no-sandbox`. Evidence anchors: `crates/tools-mcp-webfetch/src/webfetch/cache.rs::write_cache`, `crates/tools-mcp-webfetch/src/webfetch/cache.rs::cache_root`, `crates/tools-mcp-webfetch/src/webfetch/browser.rs::spawn_browser`, `crates/tools-mcp-codequery/src/codequery_cache.rs`.
...
| --- | --- | --- | --- | --- |
| JSON-RPC tool invocation | `mcp/tools/call` over stdin/stdout | Client -> MCP router | Generic control plane for all higher-risk tools. | `apps/tools-mcp-server/src/mcp_server.rs::dispatch_jsonrpc_request` |
| Filesystem mutation tools | `Write`, `Edit`, `Delete`, `Move`, `Copy` | Router -> local filesystem | Paths are caller-controlled; no central cwd confinement found. | `crates/tools-mcp-local/src/tools/write.rs::handle_write`, `crates/tools-mcp-local/src/smart_file_edit/mod.rs::handle_edit`, `crates/tools-mcp-local/src/tools/delete.rs::handle_delete`, `crates/tools-mcp-local/src/tools/fileops.rs::handle_move` |
| Git mutation tools | `GitRestore`, `GitCheckout`, `GitStash`, `GitAdd`, `GitCommit`, `GitDiff` with `output_dir` | Router -> `git` subprocess / worktree | Can discard work, change branches, stage, commit, or write patch files. | `crates/tools-mcp-git/src/git/handlers/mutating.rs`, `crates/tools-mcp-git/src/git/mod.rs::run_git` |
| PowerShell execution | `Pwsh` | Router -> `pwsh` subprocess | Highest-impact local execution surface. | `crates/tools-mcp-local/src/tools/pwsh.rs::execute_pwsh`, `crates/tools-mcp-core/src/process_utils.rs::run_pwsh_command` |
| Remote fetch | `WebFetch` | Router -> remote HTTP/browser | Hardened for SSRF and robots, but remote content remains attacker-controlled input. | `crates/tools-mcp-webfetch/src/webfetch_tool.rs::handle_webfetch`, `crates/tools-mcp-webfetch/src/webfetch/http.rs::fetch_document` |
...
| OpenAI code indexing/search | `CodeQuery` | Router -> OpenAI API | Auto-discovers and uploads local repo files by default. | `crates/tools-mcp-codequery/src/tool_handler.rs::handle_code_query`, `crates/openai-file-search-core/src/files.rs::upload_file` |
| Local file discovery/read/search | `Read`, `Search`, `Glob`, `ListDir` | Router -> local filesystem / subprocess | Broad local visibility increases reconnaissance value for later misuse. | `crates/tools-mcp-local/src/tools/handlers/read_file.rs::handle_read_file`, `crates/tools-mcp-local/src/tools/handlers/ripgrep.rs::handle_ripgrep`, `crates/tools-mcp-local/src/tools/glob.rs::handle_glob`, `crates/tools-mcp-local/src/tools/fileops.rs::handle_listdir` |
...
| --- | --- | --- |
| `apps/tools-mcp-server/src/mcp_server.rs` | Single tool-call control plane without internal authz gates. | TM-001, TM-002, TM-007 |
| `apps/tools-mcp-server/src/composition.rs` | Enumerates every registered runtime capability exposed to callers. | TM-001, TM-002, TM-003 |
| `crates/tools-mcp-local/src/tools/pwsh.rs` | Exposes arbitrary PowerShell execution to MCP callers. | TM-001 |
| `crates/tools-mcp-core/src/process_utils.rs` | Core subprocess execution layer for PowerShell and other commands. | TM-001, TM-005 |
| `crates/tools-mcp-local/src/tools/write.rs` | Creates caller-chosen files without cwd confinement. | TM-002 |
| `crates/tools-mcp-local/src/tools/delete.rs` | Deletes caller-chosen files without cwd confinement. | TM-002 |
| `crates/tools-mcp-local/src/tools/fileops.rs` | Move/copy/list operations accept arbitrary paths and can mutate directories recursively. | TM-002 |
| `crates/tools-mcp-local/src/smart_file_edit/mod.rs` | Edit path is powerful and lacks a shared path policy despite stale-file protections. | TM-002 |
| `crates/tools-mcp-git/src/git/handlers/mutating.rs` | Houses destructive git operations and patch-output writing. | TM-003 |
| `crates/tools-mcp-git/src/git/mod.rs` | Executes `git` with caller-controlled working directories. | TM-003 |
| `crates/tools-mcp-codequery/src/tool_handler.rs` | Auto-discovers local files, resolves vector stores, and drives OpenAI egress defaults. | TM-004 |
| `crates/openai-file-search-core/src/reindex.rs` | Implements upload/query orchestration and the off-host confidentiality boundary. | TM-004 |
| `crates/tools-mcp-webfetch/src/webfetch/http.rs` | Main SSRF, DNS pinning, redirect, and robots enforcement boundary. | TM-005, TM-007 |
| `crates/tools-mcp-webfetch/src/webfetch/browser.rs` | Browser subprocess configuration includes `--no-sandbox`. | TM-005 |
| `crates/tools-mcp-webfetch/src/webfetch/cache.rs` | Cache persistence has no TTL or size cap. | TM-006 |
| `crates/tools-mcp-webfetch/src/webfetch/extract.rs` | Output sanitization is content-cleaning, not a true prompt-injection control. | TM-007 |
