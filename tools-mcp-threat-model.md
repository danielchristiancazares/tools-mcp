...
- In scope:
  - Runtime MCP server and tool routing in `src/main.rs`, `src/adapters/inbound/mcp_server.rs`, and `src/composition.rs`.
...
## System model
### Primary components
- `MCP transport/router`: Reads JSON-RPC from stdin/stdout and dispatches `mcp/tools/call` requests without an internal authn/authz layer. Evidence anchors: `src/main.rs::main`, `src/adapters/inbound/mcp_server.rs::dispatch_jsonrpc_request`.
- `Tool registry/control plane`: Registers all callable tools, including mutating git, file, network, and process-execution tools. Evidence anchors: `src/composition.rs::build_tool_registry`, `src/tool_registry.rs::ToolRegistry::call`.
- `Local filesystem and git layer`: Provides `Read`, `Edit`, `Write`, `Delete`, `Move`, `Copy`, `ListDir`, `Glob`, and multiple git helpers over caller-supplied paths and working directories. Evidence anchors: `src/tools/write.rs::handle_write`, `src/tools/delete.rs::handle_delete`, `src/tools/fileops.rs::handle_move`, `src/smart_file_edit/mod.rs::handle_edit`, `src/git/handlers.rs`.
- `Process execution layer`: Executes arbitrary PowerShell commands and git subprocesses with timeouts and output caps. Evidence anchors: `src/tools/pwsh.rs::execute_pwsh`, `src/process_utils.rs::run_pwsh_command`, `src/git/mod.rs::run_git`.
- `WebFetch pipeline`: Fetches remote pages over HTTP or headless Chrome with SSRF validation, robots checks, extraction, chunking, and disk caching. Evidence anchors: `src/application/webfetch_tool.rs::handle_webfetch`, `src/webfetch/mod.rs::run_fetch`, `src/webfetch/http.rs::fetch_document`, `src/webfetch/browser.rs::spawn_browser`.
- `CodeQuery/OpenAI integration`: Auto-discovers local files, resolves or creates vector stores, uploads indexable files to OpenAI, and queries them. Evidence anchors: `src/application/codequery_tool.rs::handle_code_query`, `src/application/codequery_tool.rs::discover_default_file_paths`, `src/lib.rs::upload_file`, `src/lib.rs::code_query`.
...
- Trusted local MCP client -> MCP server router
  ...
  - Validation and enforcement: request parsing and tool schema validation only; no in-repo caller authentication or authorization. Evidence anchors: `src/main.rs::main`, `src/adapters/inbound/mcp_server.rs::dispatch_jsonrpc_request`.
- MCP server -> local filesystem and git worktree
  ...
  - Validation and enforcement: no central confinement to current working directory/subdirectories for mutation paths. Evidence anchors: `src/tools/write.rs::handle_write`, `src/tools/delete.rs::handle_delete`, `src/tools/fileops.rs::handle_move`, `src/smart_file_edit/mod.rs::apply_snippet_edit_impl`, `src/git/mod.rs::run_git`.
- MCP server -> PowerShell process
  ...
  - Validation and enforcement: no in-repo command allowlist or env-gate check found; deployment gate is external. Evidence anchors: `src/tools/pwsh.rs::execute_pwsh`, `src/process_utils.rs::run_pwsh_command`.
...
- MCP server -> OpenAI API via CodeQuery
  ...
  - Validation and enforcement: no secret-aware upload policy or approval step before sending repo contents off-host. Evidence anchors: `src/application/codequery_tool.rs::handle_code_query`, `src/application/codequery_tool.rs::discover_default_file_paths`, `src/lib.rs::upload_file`, `src/lib.rs::code_query`.
- MCP server -> local cache and browser state
  ...
  - Validation and enforcement: web cache has no expiry or size cap; browser runs with `--no-sandbox`. Evidence anchors: `src/webfetch/cache.rs::write_cache`, `src/webfetch/cache.rs::cache_root`, `src/webfetch/browser.rs::spawn_browser`, `src/codequery_cache.rs`.
...
| --- | --- | --- | --- | --- |
| JSON-RPC tool invocation | `mcp/tools/call` over stdin/stdout | Client -> MCP router | Generic control plane for all higher-risk tools. | `src/adapters/inbound/mcp_server.rs::dispatch_jsonrpc_request` |
| Filesystem mutation tools | `Write`, `Edit`, `Delete`, `Move`, `Copy` | Router -> local filesystem | Paths are caller-controlled; no central cwd confinement found. | `src/tools/write.rs::handle_write`, `src/smart_file_edit/mod.rs::handle_edit`, `src/tools/delete.rs::handle_delete`, `src/tools/fileops.rs::handle_move` |
| Git mutation tools | `GitRestore`, `GitCheckout`, `GitStash`, `GitAdd`, `GitCommit`, `GitDiff` with `output_dir` | Router -> `git` subprocess / worktree | Can discard work, change branches, stage, commit, or write patch files. | `src/git/handlers.rs`, `src/git/mod.rs::run_git` |
| PowerShell execution | `Pwsh` | Router -> `pwsh` subprocess | Highest-impact local execution surface. | `src/tools/pwsh.rs::execute_pwsh`, `src/process_utils.rs::run_pwsh_command` |
| Remote fetch | `WebFetch` | Router -> remote HTTP/browser | Hardened for SSRF and robots, but remote content remains attacker-controlled input. | `src/application/webfetch_tool.rs::handle_webfetch`, `src/webfetch/http.rs::fetch_document` |
...
| OpenAI code indexing/search | `CodeQuery` | Router -> OpenAI API | Auto-discovers and uploads local repo files by default. | `src/application/codequery_tool.rs::handle_code_query`, `src/lib.rs::upload_file` |
| Local file discovery/read/search | `Read`, `Search`, `Glob`, `ListDir` | Router -> local filesystem / subprocess | Broad local visibility increases reconnaissance value for later misuse. | `src/tools/handlers/read_file.rs::handle_read_file`, `src/tools/handlers/ripgrep.rs::handle_ripgrep`, `src/tools/glob.rs::handle_glob`, `src/tools/fileops.rs::handle_listdir` |
...
| `src/tools/delete.rs` | Deletes caller-chosen files without cwd confinement. | TM-002 |
| `src/tools/fileops.rs` | Move/copy/list operations accept arbitrary paths and can mutate directories recursively. | TM-002 |
| `src/smart_file_edit/mod.rs` | Edit path is powerful and lacks a shared path policy despite stale-file protections. | TM-002 |
| `src/git/handlers.rs` | Houses destructive git operations and patch-output writing. | TM-003 |
| `src/git/mod.rs` | Executes `git` with caller-controlled working directories. | TM-003 |
| `src/application/codequery_tool.rs` | Auto-discovers local files, resolves vector stores, and drives OpenAI egress defaults. | TM-004 |
| `src/lib.rs` | Implements upload/query behavior and the off-host confidentiality boundary. | TM-004 |
| `src/webfetch/http.rs` | Main SSRF, DNS pinning, redirect, and robots enforcement boundary. | TM-005, TM-007 |
| `src/webfetch/browser.rs` | Browser subprocess configuration includes `--no-sandbox`. | TM-005 |
| `src/webfetch/cache.rs` | Cache persistence has no TTL or size cap. | TM-006 |
