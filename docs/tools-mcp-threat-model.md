...
- MCP server -> local cache and browser state
  ...
  - Validation and enforcement: web cache has no expiry or size cap; browser runs with `--no-sandbox`. Evidence anchors: `tools-mcp-webfetch/src/webfetch/cache.rs::write_cache`, `tools-mcp-webfetch/src/webfetch/cache.rs::cache_root`, `tools-mcp-webfetch/src/webfetch/browser.rs::spawn_browser`.
...
4. Browser-mediated host compromise
...
5. Cache persistence and local exposure
...
6. Prompt-injected remote content drives unsafe tool chaining
...
- Reflected user clarifications: local trusted deployment, external `Pwsh` env gate, and desired cwd-only modification policy.
