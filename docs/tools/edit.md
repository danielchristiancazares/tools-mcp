...
`Edit` performs a surgical, exact-substring replacement in a single text file. It normalizes both file and snippets to a canonical LF view for matching, then writes back the file with its **original dominant** line-ending style (`LF`, `CRLF`, or `CR`) applied to the new content. The tool enforces a workspace-root path policy, **requires the file to have been read in the current server session** (a mandatory snapshot guard, see §3.3), and supports an optional line-range `match_hint` to disambiguate between multiple occurrences of the same snippet. The tool is owned by the `tools-mcp-local` crate; the entry point is `handle_edit` in `tools-mcp-local/src/smart_file_edit/mod.rs`.

### 3.3 Mandatory Read Snapshot

`Edit` does not accept any caller-provided hash. Instead the server records an in-memory snapshot (the file's SHA-256) every time `Read` returns content for a file, keyed by canonical path and scoped to the server process. On every `Edit`:

- If no snapshot exists for the file (it was never read in this session), the edit is rejected with `status: "no_snapshot"` and the file is NOT modified.
- If a snapshot exists but the file on disk no longer matches it (the file changed since it was read), the edit is rejected with `status: "stale_file"` and the file is NOT modified.
- After a successful edit, the server refreshes the snapshot to the bytes it just wrote, so a chain of edits to the same file proceeds without re-reading.

Nothing is copied between tool calls: the caller never passes a hash, and the server is the sole source of truth for what the agent last observed.
...
- **Mandatory read snapshot.** Before any write, `handle_edit` looks up the server-recorded snapshot for the resolved path. If none exists, it returns `status: "no_snapshot"` and does NOT write. The expected hash is taken from the server's snapshot, never from caller input. Locked in by `public_edit_without_snapshot_is_rejected`.
- **`deny_unknown_fields`.** The outer `SimpleEditRequest` and the inner `MatchHint` both reject unknown properties. Unknown fields produce `"invalid arguments: ..."`.
- **Newline-style preservation.** The replacement bytes MUST be written with the file's dominant newline style as detected by `NewlineStats::default_kind()`. When the file has no detected newlines, default to `LF`. Tie-breaking prefers `CRLF` > `LF` > `CR`.
- **Snippet normalization for matching.** Both `old_snippet` and `new_snippet` are normalized to LF before matching. A caller may pass CRLF-terminated snippets and they will still match an LF file. Locked in by `apply_snippet_edit_accepts_crlf_snippets_from_clients`.
- **Strict `match_hint`.** When `match_hint` is provided, search is restricted to the canonical bytes inside the hint window; if no match exists inside the hint, the result is `status: "no_match"` and the file is NOT modified. There is NO fallback search outside the hint. Locked in by `match_hint_selects_correct_occurrence_and_is_strict`.
- **First-match replacement (no `replace_all`).** Even when `old_snippet` appears multiple times in the file, `compute_match_range` returns only the first match. The remaining occurrences are unchanged. There is no input flag to widen this behavior.
- **Stale-file refusal.** If the file on disk differs from the server snapshot taken at the last `Read` (or last successful `Edit`), the handler returns `status: "stale_file"` and does NOT write the file. Locked in by `public_edit_rejects_stale_file_when_changed_after_read` and `apply_snippet_edit_rejects_stale_file_hash_and_does_not_modify_file`.
- **Snapshot refresh after edit.** After a successful write, the server records the new file hash as the snapshot, so chained edits do not require re-reading. Locked in by `public_edit_refreshes_snapshot_for_chained_edits`.
- **Hash format.** Snapshot hashes are formatted `"sha256:<64-hex-chars>"`, identical to `FileModel` hashes.
- **`status` reflects success class.** The response payload `status` field MUST be exactly one of `"ok"`, `"no_match"`, `"stale_file"`, `"no_snapshot"`. `"ok"` returns `isError: false`; the others return `isError: true`.
...
- MUST NOT write a file that was not read in this session (`no_snapshot`). The file is left exactly as it was.
- MUST NOT write a file that changed on disk since it was read (`stale_file`). The file is left exactly as it was.
- MUST NOT accept a caller-supplied file hash; the staleness guard uses only the server's own read snapshot.
...
- **Mandatory staleness check, no copying.** A successful `Read` records the file's hash server-side; `Edit` refuses unless the file was read and still matches, and refreshes the snapshot after writing. This keeps a read → edit → edit chain coherent and race-safe without the caller ever copying a hash between tool calls.
...
There is no `file_hash` input. Staleness is enforced from the server's own read snapshot (§3.3); the caller passes no hashes.

The schema sets `"additionalProperties": false`; the request type uses `#[serde(deny_unknown_fields)]`. The inner `MatchHint` also uses `deny_unknown_fields`.

> Schema source: `tools-mcp-local/src/tools/edit.rs`

### 6.2 Behavior

1. **Parse arguments** — Deserialize into `SimpleEditRequest`; on failure return the `parse_args` error envelope.
2. **Validate `path` non-empty** — `validation::validate_non_empty(&req.path, "path", None)`; whitespace-only paths produce `"path is required (non-empty string)"`.
3. **Validate `old_snippet` non-empty** — Reject empty `old_snippet` with the LF-remediation message.
4. **Resolve path under workspace** — `path_policy::resolve_existing_file(&req.path, "path")`. Rejects nonexistent files, directories, paths outside the workspace root, and symlinks that escape the root.
5. **Snapshot lookup (mandatory)** — Look up the server snapshot for the resolved path via `edit_snapshot::get`. If absent, return `status: "no_snapshot"` and do NOT write. Otherwise the snapshot hash becomes the expected `file_hash` passed into the engine.
6. **Hand off to `apply_snippet_edit_impl`** — Wraps the request in `ApplySnippetEditRequest` with the snapshot hash as `file_hash`.
7. **Snippet normalization** — `normalize_newlines_to_lf` converts CRLF and lone CR to LF in both snippets.
8. **Load file model** — `FileModel::from_path(&path)` reads bytes once, computes SHA-256 hash, splits into lines with newline detection, and builds the LF-canonical view with bidirectional offset boundaries.
9. **Stale-file check** — Compare the snapshot hash against `model.hash`; on mismatch return `SnippetStatusKind::StaleFile` with payload `{"action":"apply_snippet_edit","status":"stale_file","message":"file hash mismatch, refresh region before applying changes","expected_file_hash":<snapshot>,"current_file_hash":<actual>}`. On success, the snapshot is refreshed to the post-edit hash.
...
**No-snapshot — `status: "no_snapshot"` (`isError: true`):**

Returned when the file was not read in this server session, so there is no snapshot to verify against:
...
| File never read this session | `isError: true`, JSON `status: "no_snapshot"` | See §6.3 no-snapshot shape |
| File changed since it was read | `isError: true`, JSON `status: "stale_file"` | See §6.3 stale-file shape |
...
- **Mandatory snapshot, no copied secrets.** Staleness is enforced from the server's in-memory read snapshot, not a caller-supplied hash. The agent cannot bypass the check by omitting a field, and never transports a hash between calls. A second writer whose file changed since it read gets `stale_file` and must re-read.
...
After the file is read (recording a snapshot) it changes on disk, then an Edit is attempted:
...
Returns `status: "stale_file"`; the file is not modified. Locked in by `public_edit_rejects_stale_file_when_changed_after_read`.

### 10.6 No-snapshot rejection

An Edit on a file that was never read in this session:
...
Returns `status: "no_snapshot"`; the file is not modified. Locked in by `public_edit_without_snapshot_is_rejected`.
...
|---|---|---|
| `public_edit_without_snapshot_is_rejected` | `tools-mcp-local/src/smart_file_edit/mod.rs` | Editing a file that was not read this session returns `status: "no_snapshot"` and preserves content. |
| `public_edit_rejects_stale_file_when_changed_after_read` | `tools-mcp-local/src/smart_file_edit/mod.rs` | A file changed after its read snapshot returns `status: "stale_file"` and preserves content. |
| `public_edit_refreshes_snapshot_for_chained_edits` | `tools-mcp-local/src/smart_file_edit/mod.rs` | A successful edit refreshes the snapshot so a second edit needs no re-read. |
| `public_edit_forwards_region_id_to_success_payload` | `tools-mcp-local/src/smart_file_edit/mod.rs` | `region_id` echoes verbatim on success. |
| `public_edit_rejects_parent_traversal_outside_workspace` | `tools-mcp-local/src/smart_file_edit/mod.rs` | `..` traversal blocked by path policy. |
| `public_edit_canonicalizes_symlinked_file_before_writing` (Unix) | `tools-mcp-local/src/smart_file_edit/mod.rs` | Symlink inside workspace resolves to its target file before writing. |
...
| 5 | Is `Edit` required to be preceded by a `Read`? | Yes. `Edit` requires a server-recorded read snapshot for the file; without one it returns `status: "no_snapshot"`. A successful `Edit` refreshes the snapshot, so a chain of edits to the same file does not need an intervening `Read`. Snapshots are process-scoped, so a `Read` and `Edit` issued to separate short-lived server processes do not share state. |
